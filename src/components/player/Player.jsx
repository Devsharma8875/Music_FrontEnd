...
const Player = () => {
  ...
  // get songs audio url
  // Replace your getSongAudioUrls function with this:
  const getSongAudioUrls = async () => {
    setAudioLoading(true);
    try {
      const { audioFormatHigh, audioFormatLow } = await getAudioUrls({ id });

      setPlayerState({
        ...playerState,
        url: audioFormat === "high" ? audioFormatHigh : audioFormatLow,
        playing: autoPlay,
      });
      ...
      if (activeToggle === "audio") {
        ...
        // Automatically switch to video if audio fails
        setActiveToggle("video");
      }
    } finally {
