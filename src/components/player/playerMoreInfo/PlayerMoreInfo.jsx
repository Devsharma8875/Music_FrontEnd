...
const PlayerMoreInfo = ({
  ...
  // Remove skip option here, just call query
  const { isSuccess, isLoading, isError } = useGetServerStatusQuery();

  const [isDownloadStart, setIsDownloadStart] = useState(false);
  const [isDownloadServerActive, setIsDownloadServerActive] = useState(true);

  useEffect(() => {
    if (isSuccess) {
      setIsDownloadServerActive(true);
    }
  }, [isSuccess]);
        ...
        >
          <span className="player-more-info-icons">
            {!isDownloadStart ? (
              ...
              <DownloadIcon width={20} height={20} className="no-spin" />
            )}
          </span>
          {"Download"}
        </button>
